...
$("#ratedecksubmit").on('click',function(){
	if($("#ratedeck").val().trim() == "") {warnInvalid($("#ratedeck"),_("Call Deck name can not be blank!")); return false;};
		var result = $.ajax({
			url: "ajax.php?module=callaccounting",
			type: 'POST',
			async: false,
			data: { command: 'addnewcd', module: "callaccounting", cdname: $("#ratedeck").val()}
		});
		obj = JSON.parse(result.responseText);
		console.log(obj);
		if (obj && obj.message != null) {
			alert(obj.message);
		}
	});
});
...
function linkFormatter(value, row, index){
	html = '<a href="?display=callaccounting&action=editcd&rate_deck_id='+encodeURIComponent(row['id'])+'"><i class="fa fa-pencil" title="Edit Dial Patterns"></i></a>&nbsp;';
	html = html +' <a href="?display=callaccounting&action=cdrlist&rate_deck_id='+row['id']+'"> <i class="fa fa-table" title="View CDRs"> </i></a>&nbsp;&nbsp;';
	html = html +' <a href="?display=callaccounting&action=delete&id='+row['id']+'" class="delAction"> <i class="fa fa-remove " title="Delete"></i></a>&nbsp;&nbsp;';
	return html;
}

function linkFormatterpattern(value, row, index){
	html = '<a href="?display=callaccounting&action=editpattern&rate_deck_id='+row['rate_deck_id']+'&id='+encodeURIComponent(row['id'])+'"><i class="fa fa-pencil fa-1x" title="Edit"></i></a>&nbsp;';
	html = html +' <a href="?display=callaccounting&action=deletepattern&rate_deck_id='+row['rate_deck_id']+'&id='+row['id']+'" class="delAction"> <i class="fa fa-remove fa-1x" title="Delete"></i></a>&nbsp;&nbsp;';
	return html;
}
