...
class FPDF
{
protected $page = 0;               // current page number
protected $n = 2;                  // current object number
protected $offsets;            // array of object offsets
protected $buffer = '';             // buffer holding in-memory PDF
protected $pages = [];              // array containing pages
protected $state = 0;              // current document state
...
protected $lasth = 0;              // height of last printed cell
...
protected $underline = false;          // underlining flag
protected $CurrentFont;        // current font info
protected $FontSizePt = 12;         // current font size in points
protected $FontSize;           // current font size in user unit
protected $DrawColor = '0 G';          // commands for drawing color
protected $FillColor = '0 g';          // commands for filling color
protected $TextColor = '0 g';          // commands for text color
protected $ColorFlag = false;          // indicates whether fill and text colors are different
protected $WithAlpha = false;          // indicates whether alpha channel is used
protected $ws = 0;                 // word spacing
protected $images = [];             // array of used images
protected $PageLinks;          // array of links in pages
protected $links = [];              // array of internal links
protected $AutoPageBreak;      // automatic page breaking
protected $PageBreakTrigger;   // threshold used to trigger page breaks
protected $InHeader = false;           // flag set when processing header
protected $InFooter = false;           // flag set when processing footer
...
{
	// Title of document
	$this->metadata['Title'] = $isUTF8 ? $title : utf8_encode((string) $title);
}

function SetAuthor($author, $isUTF8=false)
{
	// Author of document
	$this->metadata['Author'] = $isUTF8 ? $author : utf8_encode((string) $author);
}

function SetSubject($subject, $isUTF8=false)
{
	// Subject of document
	$this->metadata['Subject'] = $isUTF8 ? $subject : utf8_encode((string) $subject);
}

function SetKeywords($keywords, $isUTF8=false)
{
	// Keywords of document
	$this->metadata['Keywords'] = $isUTF8 ? $keywords : utf8_encode((string) $keywords);
}

function SetCreator($creator, $isUTF8=false)
{
	// Creator of document
	$this->metadata['Creator'] = $isUTF8 ? $creator : utf8_encode((string) $creator);
}
...
function AddFont($family, $style='', $file='')
{
	...
	$info['i'] = (is_countable($this->fonts) ? count($this->fonts) : 0)+1;
	if(!empty($info['file']))
	{
		// Embedded font
