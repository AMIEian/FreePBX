...
class TTFParser
{
	...
	function AddGlyph($id)
	{
		...
		{
			$this->glyphs[$id]['ssid'] = is_countable($this->subsettedGlyphs) ? count($this->subsettedGlyphs) : 0;
	...
	function BuildCmap()
	{
		...
		for($i=1;$i<(is_countable($chars) ? count($chars) : 0);$i++)
		{
	...
	{
		$this->LoadTable('hhea');
		$numberOfHMetrics = is_countable($this->subsettedGlyphs) ? count($this->subsettedGlyphs) : 0;
		$data = substr_replace((string) $this->tables['hhea']['data'], pack('n',$numberOfHMetrics), 4+15*2, 2);
		...
		foreach($this->subsettedGlyphs as $id)
		{
			...
			if(isset($glyph['components']))
			{
				...
				foreach($glyph['components'] as $offset=>$cid)
				{
					...
					$glyph_data = substr_replace((string) $glyph_data, pack('n',$ssid), $offset, 2);
				}
			}
			$data .= $glyph_data;
		}
	...
	{
		$this->LoadTable('maxp');
		$numGlyphs = is_countable($this->subsettedGlyphs) ? count($this->subsettedGlyphs) : 0;
		$data = substr_replace((string) $this->tables['maxp']['data'], pack('n',$numGlyphs), 4, 2);
	...
	function BuildPost()
	{
		...
		{
			// Version 2.0
			$numberOfGlyphs = is_countable($this->subsettedGlyphs) ? count($this->subsettedGlyphs) : 0;
			$numNames = 0;
			$names = '';
	...
	function BuildFont()
	{
		...
		foreach($tags as $tag)
		{
			...
			$offset += strlen((string) $this->tables[$tag]['data']);
		}
		...
		$a = unpack('n2', (string) $this->CheckSum($s));
		$high = 0xB1B0 + ($a[1]^0xFFFF);
		$low = 0xAFBA + ($a[2]^0xFFFF) + 1;
		$checkSumAdjustment = pack('nn', $high+($low>>16), $low);
		$this->tables['head']['data'] = substr_replace((string) $this->tables['head']['data'], $checkSumAdjustment, 8, 4);
	...
	function SetTable($tag, $data)
	{
		...
		$n = $length % 4;
		if($n>0)
			$data = str_pad((string) $data, $length+4-$n, "\x00");
