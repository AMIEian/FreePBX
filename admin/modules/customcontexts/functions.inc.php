  ...
  switch($engine) {
	...
	foreach ($results as $val) {
		...
		foreach (array_keys($dialpattern) as $key) {
			...
			if (isset($dialpattern[$key][0]) && $dialpattern[$key][0] == "_") {
...
function customcontexts_customcontexts_configprocess() {
	...
	$faildest= isset($_REQUEST["goto0"])?($_REQUEST[$_REQUEST['goto0'].'0'] ?? null):null;
	$featurefaildest= isset($_REQUEST["goto1"])?($_REQUEST[$_REQUEST['goto1'].'1'] ?? null):null;
