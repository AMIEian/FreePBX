$(document).ready(function() {
	...
	ldapDialStringSettings();
	$("[name='ldapDialStringsToggle']").change(function(){
		ldapDialStringSettings();
	});
	...
	// Add LDAP Dial String Attributes
	$(document).on('click',"a[id^='adddialstringrow']",function(e) {
		e.preventDefault();
		var curRow = $("tr[id^='ldapdsrow']").last();
		var id = $("tr[id^='ldapdsrow']").length++;
		var newhtml = '';
		newhtml +=	'<tr id="ldapdsrow'+id+'">';
		newhtml +=		'<td class="ldapdsid">';
		newhtml +=			'<div class="input-group">';
		newhtml += 				'<input type="hidden" id="ldapDialStrings_uid_'+id+'" name="ldapDialStrings_uid[]" class="form-control " value="" >';
		newhtml += 				'<input placeholder="LDAP Dial String ID" type="text" id="ldapDialStrings_id_'+id+'" name="ldapDialStrings_id[]" class="form-control " value="" >';
		newhtml +=			'</div>';
		newhtml +=		'</td>';
		newhtml +=		'<td class="ldapdslabel">';
		newhtml +=			'<div class="input-group">';
		newhtml +=  			'<input placeholder="LDAP Dial String Label" type="text" id="ldapDialStrings_label_'+id+'" name="ldapDialStrings_label[]" class="form-control" value="" >';
		newhtml +=			'</div>';
		newhtml +=		'</td>';
		newhtml +=		'<td>';
		newhtml += 			'<div class="actions">';
		newhtml +=				'<a href="#" id="adddialstringrow'+id+'"><i class="fa fa-plus" data-toggle="tooltip" title="Add new LDAP Dial Strings"></i></a>';
		newhtml +=				'<span style="font-weight:bold"> | </span>';
		newhtml +=				'<a href="#" id="deldialstringrow'+id+'"><i class="fa fa-trash" data-toggle="tooltip" title="Delete LDAP Dial Strings"></i></a>';
		newhtml +=			'</div>';
		newhtml +=		'</td>';
		newhtml +=	'</tr>';
		curRow.parent().append(newhtml);
	...
	//Del LDAP Dial String Attributes
	$(document).on('click',"a[id^='deldialstringrow']",function(e) {
		e.preventDefault();
		var curRow = $(this).closest('tr');
		var dialStringUId = curRow.find("[name^='ldapDialStrings_uid']").val();
		var templateName = $("input[name='template_name']").val();
		delLDAPDialStr(dialStringUId, templateName);
		curRow.remove();
	});
});
...
function dpmaAppsSupportNotification (model) {
	...
	let dpmaApps = ["App-Contacts", "App-Parking", "App-Status", "APP-Follow Me", "APP-Time Conditions", "APP-Call Flow"];
...
function toggleLDAPPassword() {
	const togglePassword = document.querySelector('#toggleLDAPPassword');
	const passwordField = document.querySelector('#ldapPPassword');
...
function ldapDialStringSettings() {
	if($('#ldapDialString-enable').is(':checked')) {
		$('.section[data-id=ldapDailStrings]').show("slow");
		document.getElementById("showLdapDailStrings").style.display = "";
	} else {
		$('.section[data-id=ldapDailStrings]').hide("slow");
		document.getElementById("showLdapDailStrings").style.display = "none";
	}
}

function delLDAPDialStr(uid, templateName) {
	var result = $.ajax({
		url: "ajax.php?module=endpoint&command=delLDAPDialStr&uid="+uid+"&templateName="+templateName,
		type: 'POST',
		async: false
	});
	return result;
