$(document).ready(function() {
	...
	$('#send-button').click(function(){
		...
		var cTransport = $('select[name="cTransport"]').val();
		var dataString = 'display=endpoint&view=save_custom&cExt=' + cExt + '&cSecret=' + cSecret + '&cLabel=' + cLabel + '&cDestination=' + cDestination + '&cSipPort=' + cSipPort  + '&cTransport=' + cTransport;
		$.ajax({
			type: "POST",
			url: "config.php",
			data: dataString,
	...
	$(document).on('change', '.content select[name^="brand["], select[name="brand_exist"]', function() {
        ...
        } else if($(this).val() == 'Fanvil'){
		$.each(fanvil, function(key, value){
			ext_list = ext_list + '<option value="' + value + '">' + value + '</option>';
		});
		$.each(fanvilModels, function(key, value){
			mod_list = mod_list + '<option value="' + value + '">' + value + '</option>';
		});
	}
