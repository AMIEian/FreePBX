		...
		dz.on("success", function(file) {
			...
			try {
				...
				if(obj.status) {
					...
					// Handle both 'error' and 'message' fields for backward compatibility
					var errorMsg = obj.error || obj.message || 'Unknown error occurred';
					alert(errorMsg);
				}
			} catch (e) {
				console.error('Failed to parse response:', result.responseText);
				alert('Invalid response from server: ' + result.responseText);
			}
		});
	});
