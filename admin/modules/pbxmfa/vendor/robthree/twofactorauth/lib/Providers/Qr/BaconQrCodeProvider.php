...
class BaconQrCodeProvider implements IQRCodeProvider
{
     ...
     */
    public function __construct(private $borderWidth = 4, $backgroundColour = '#ffffff', $foregroundColour = '#000000', $format = 'png')
    {
    ...
    public function getMimeType()
    {
        return match ($this->format) {
            ...
            default => throw new \RuntimeException(sprintf('Unknown MIME-type: %s', $this->format)),
        };
    }
    ...
    private function getQRCodeByBackend($qrText, $size, ImageBackEndInterface $backend)
    {
        ...
        if (is_array($this->foregroundColour) && is_array($this->backgroundColour)) {
            $rendererStyleArgs = [...$rendererStyleArgs, null, null, Fill::withForegroundColor(
