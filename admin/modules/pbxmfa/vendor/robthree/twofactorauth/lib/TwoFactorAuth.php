...
class TwoFactorAuth
{
     ...
     */
    public function __construct(private $issuer = null, $digits = 6, $period = 30, $algorithm = 'sha1', private ?\RobThree\Auth\Providers\Qr\IQRCodeProvider $qrcodeprovider = null, private ?\RobThree\Auth\Providers\Rng\IRNGProvider $rngprovider = null, private ?\RobThree\Auth\Providers\Time\ITimeProvider $timeprovider = null)
    {
     ...
     * @return string
     */
    public function getQRCodeImageAsDataUri($label, $secret, mixed $size = 200)
    {
    ...
    public function ensureCorrectTime(array $timeproviders = null, $leniency = 5)
    {
        if ($timeproviders === null) {
            $timeproviders = [new NTPTimeProvider(), new HttpTimeProvider()];
        }
        ...
        foreach ($timeproviders as $t) {
            ...
            if (abs($timeprovider->getTime() - $t->getTime()) > $leniency) {
                throw new TwoFactorAuthException(sprintf('Time for timeprovider is off by more than %d seconds when compared to %s', $leniency, $t::class));
            }
        }
    }
