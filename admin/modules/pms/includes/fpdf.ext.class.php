...
class PDF extends \FPDF
{
	...
	protected $U = 0;
	protected $HREF = '';
	protected $fontList = ['Times', 'Courier'];
	...
	function hex2dec($color = "#000000"){
		$tbl_color = [];
		$tbl_color['R']=hexdec(substr((string) $color, 1, 2));
		$tbl_color['G']=hexdec(substr((string) $color, 3, 2));
		$tbl_color['B']=hexdec(substr((string) $color, 5, 2));
		return $tbl_color;
	}
	...
	function WriteHTML($html,$bi)
	{
		...
		$this->bi=$bi;
		if ($bi)
			$html=strip_tags((string) $html,"<a><img><p><br><font><tr><blockquote><h1><h2><h3><h4><pre><red><blue><ul><li><hr><b><i><u><strong><em>"); 
		else
			$html=strip_tags((string) $html,"<a><img><p><br><font><tr><blockquote><h1><h2><h3><h4><pre><red><blue><ul><li><hr>"); 
	...
	function PutMainTitle($title) {
		if (strlen((string) $title)>55)
			$title=substr((string) $title,0,55)."...";
	...
	function PutMinorTitle($title,$url='') {
		$title=str_replace('http://','',(string) $title);
		if (strlen($title)>70)
	...
	function RoundedRect($x, $y, $w, $h, $r, $corners = '1234', $style = ''){
		...
		if (!str_contains((string) $corners, '2'))
		...
		if (!str_contains((string) $corners, '3'))
		...
		if (!str_contains((string) $corners, '4'))
		...
		if (!str_contains((string) $corners, '1'))
		{
