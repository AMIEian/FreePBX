...
*/
	...
	handler_agentstate: function(widget, uuid, data) {
		...
		if(data.currentstatus == 'paused') {
			let queuenum = $('[data-queueid="'+ uuid +'"]').attr("data-qnum");
			if((queuenum in data.paused) && data.paused[queuenum] == true) {
				$('[data-queueid="'+ uuid +'"]').html(html);
				$('.stxt-'+uuid).text(icon.text);
			}
			 else {
				icon = this.map_agent_icon('idle');
				html = "<i class='fa fa-5x "+icon.class.join(" ")+"'></i>";
				$('[data-queueid="'+ uuid +'"]').html(html);
				$('.stxt-'+uuid).text(icon.text);
			}
		} else {
