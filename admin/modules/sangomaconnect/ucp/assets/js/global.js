var SangomaconnectC = UCPMC.extend({
	/**
	...
	displaySimpleWidget: function(widget_id) {
		const self = this;
		// Optional: remove previous handler to prevent double-binding
		$(document).off('click', '#launchscd');

		// Attach click handler
		$(document).on('click', '#sett-launch-app', function () {
			self.launchSCDApp(widget_id, 'desktop');
		});
		$(document).on('click', '#launchscd', function () {
			self.launchSCDApp(widget_id, 'desktop');
		});
		$(document).on('click', '#launchwebscd', function () {
			self.launchSCDApp(widget_id, 'web');
		});
		$(document).on('click', '#sett-launchwebscd	', function () {
			self.launchSCDApp(widget_id, 'web');
		});
	},
	launchSCDApp: function(widget_id, type) {
		var widget = $(".custom-widget[data-widget_id='" + widget_id + "']");
		switch (widget.data("widget_type_id")) {
			case "sangoma_phone_desktop_client":
				$.ajax({
					type: 'POST',
					url: UCP.ajaxUrl,
					data: { module: "sangomaconnect", command: "getLoginUrl", type: type },
					dataType: 'json',
					timeout: 30000,
...
function replaceWizardIcon(){
	document.querySelectorAll('.customscdicon').forEach(icon => {
		// Create the new <i> element
		const iTag = document.createElement('i');
		iTag.classList.add('customscdicon');

		// Create the <img> element
		const img = document.createElement('img');
		img.src = '../admin/assets/sangomaconnect/images/app-icon.png';
		img.draggable = false;
		img.style.width = '25px';
		img.style.height = '25px';

		// Nest the image inside the <i>
		iTag.appendChild(img);

		// Check if the current element is inside an <a>
		const anchor = icon.closest('a');

		if (anchor) {
			anchor.title = 'Sangoma Phone Desktop Client';
			// Remove the old icon and append the new one to the anchor
			icon.remove();
			anchor.appendChild(iTag);
		} else {
			// Remove the <small> tag inside the same parent if present
...
$(document).ready(function () {
    $(document).on('click', '#sett-launchwebscd', function () {
        var widgetId = $('.custom-widget').data('widget_id');
       if (typeof widgetId !== "undefined") {
			var instance = new SangomaconnectC();
			instance.launchSCDApp(widgetId, 'web');
        } else {
            console.warn("UCP.sangomaconnect not found.");
        }
    });
});
