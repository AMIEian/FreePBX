...
function sysadmin_updateHostname() {
	...
	if (newhostname.length < 4) {
		$("#errormsg").html("<strong>Error:</strong>  Hostname too short");

	} else if(hostPattern.test(newhostname)){
		$("#errormsg").html("<strong>Error:</strong>  Invalid Hostname(Hostname should not conatins '_'");
