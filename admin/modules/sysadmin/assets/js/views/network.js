...
NetworkC = Class.extend({
	...
	loadNetworkConfig: function(fpbxconfig = false) {
		// For use in ajax
		var self = this;
		...
		$.ajax({
			...
			success: function(data) {
				if(data.otherConfig && !fpbxconfig) {
					$('#network_set_form').hide();
					$('#action-bar').hide();
					$('#intConfigModal').modal('show');
				}
				self.netconf = data;
				// Load the select
				self.updateNetworks();
	...
	updateNetworks: function() {
		...
		if (this.netconf.interfaces && Object.keys(this.netconf.interfaces).length == 0) {
			// Bad. 
...
function handleInterface(choice) {
	if(choice == 'yes') {
		window.Network.loadNetworkConfig(true);
		$('#network_set_form').show();
		$('#intConfigModal').modal('hide');
		$('#action-bar').show();
	} else {
		$('#network_set_form').show();
		$('#network_set_form :input').prop('disabled', true);
		$('#intConfigModal').modal('hide');
		$('#action-bar').hide();
	}
}