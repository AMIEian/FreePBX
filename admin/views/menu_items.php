...
if (isset($fpbx_menu) && is_array($fpbx_menu)) {	// && freepbx_menu.conf not defined
	...
	foreach($menu as $t => $cat) { //categories
		if (count($cat) == 1) {
			...
			// Hide legacy FreePBX Support link
			$href = isset($cat[0]['href']) ? $cat[0]['href'] : 'config.php?display=' . $cat[0]['display'];
			if (!empty($cat[0]['name']) && trim($cat[0]['name']) === 'FreePBX Support') {
				continue;
			}
			if (!empty($href) && strpos($href, 'freepbx.org/support-and-professional-services') !== false) {
				continue;
			}
		...
		foreach ($cat as $c => $mod) { //modules
			// Hide legacy FreePBX Support link
			if (!empty($mod['name']) && trim($mod['name']) === 'FreePBX Support') {
				continue;
			}
			...
			if (!empty($mod['display']) && trim($mod['display']) === 'wiki') {
				continue;
			}
			...
			//build default module url (href already set above for FreePBX Support check)
